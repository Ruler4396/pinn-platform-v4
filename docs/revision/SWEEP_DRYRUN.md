...
### 13.1 23:4x 红灯（统括官零机时复现）：`prepare()` 只匹前缀 ⇒ 目标父目录不存在，35 次求解会全 rc=8

他给的现象我在本机零求解复现了一模一样：改写后的 ofstream 是
`<work>/gen/contraction/C-base/contraction_2d/cfd/C-base/C-base_raw.csv`，而 `mkdir` 只建到 `<work>/gen/contraction/C-base` ⇒ `父目录 exists=False`。
**判决不会错**（`all_zero_rc=False` → 末尾 INVALID），但那趟机时白烧、E5 拿不到中位数。

修的不是"文本有没有变"，是**目标目录能不能落文件**（他点出的关键：旧控制"空操作就 INVALID"拓不到这个形状 —— 那是改得不完整，不是没改）：

1. 正则改为匹**整个字符串字面量** `"/root/dev/[^"
]*"`，主机前缀单独剥，其余层级原样保留 ⇒ `cases/<族>/cfd/<工况>/x_raw.csv` 不再被截断；
2. `prepare()` 里对每个目标 `mkdir(parents=True)` 后，**`verify_targets()` 逐条探一次"能否在其中落文件"**（建父目录 + 写探针 + 删探针），任一条失败 ⇒ `SystemExit` 点名缺哪一层，且发生在计时之前；
3. 追加"改写后不得残留任何 `/root/dev` 字面量"断言（`ofstream` 与 `cout` 回显一起改，否则 64 处只改了 32 处）；
4. 追加"改写件/产物目录不得落在仓库内"断言 —— 保护那批按旧字节取证的"5/5 逐位 diff=0"凭据；
5. `--self-test` 从四项升到**七项**，其中三条是新增的硬控制：真件 `prepare()`（C-base/B-base 各一次，零求解，父目录必须存在且在工作区内）、**必定红**（给一个缺层目标 ⇒ 必须 SystemExit）、**仓内落点必须被拒**（且不留目录）。本机实测 exit=0。

复现对照（修完后）：`C-base → <work>/gen/contraction/C-base/cases/contraction_2d/cfd/C-base/C-base_raw.csv  父目录存在=True`，弯曲族同。层级看着重复是故意的：**宁深不猜**，不去假设原作者那棵树的层数。

反对照按他 §五 的字面要求扩到**五个工况全跑**（不再只抽两个）：`C-base / C-train-1 / C-test-2 / B-base / B-train-1` 的 `prepare()` 逐个验证 ofstream 目标父目录存在且可落文件，本机零求解 exit=0（`bc60060`）。**所用变体 = 保留原层级**（`<work>/gen/<族>/<工况>/cases/<族目录>/cfd/<工况>/x_raw.csv`），不是他示例里"只留文件名"那一版 —— 目的是不假设原作者树的层数；复核请按这一种。

他确认"不用改"的三条照原样留着（每工况 7 次≈15–20 s 在 2 min 封顶内、`DEFAULT_CASES` 三收缩+两弯曲使族数闸不误触、改写件在系统临时目录不碰仓内 `.edp`）。
