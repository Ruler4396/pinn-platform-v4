#!/usr/bin/env python3
"""严格稀疏版：独立速度模型 + 独立压力模型 + 控制方程交替耦合训练入口。
...
class 输出标准化器:
    ...
    def fit(cls, values: np.ndarray) -> "输出标准化器":
        ...
        if arr.ndim != 2:
            raise ValueError(f"输出标准化器.fit expects 2D array, got shape={arr.shape}")
        ...
        for idx in range(arr.shape[1]):
            ...
            if finite.size == 0:
                raise ValueError("严格稀疏训练无法从空观测列拟合输出标准化器")
...
def 方程耦合损失(
    ...
    # 严格稀疏训练中，PDE 残差尺度不能再从 dense 真值场读取。
    # 使用由观测点拟合得到的输出标准化尺度，避免未观测标签进入训练损失。
    velocity_scale_value = float(np.linalg.norm(np.asarray(速度标准化器.std, dtype=np.float64)))
    pressure_scale_value = float(np.max(np.asarray(压力标准化器.std, dtype=np.float64)))
    velocity_scale = torch.tensor(max(velocity_scale_value, 1.0e-12), dtype=torch.float32, device=device)
    pressure_scale = torch.tensor(max(pressure_scale_value, 1.0e-12), dtype=torch.float32, device=device)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="严格稀疏版：独立速度模型 + 独立压力模型 + 控制方程交替耦合")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="输入和输出标准化只从训练观测源拟合；默认关闭以兼容旧实验",
    )
    return parser
...
def train_once(args: argparse.Namespace) -> dict:
    ...
    if args.strict_sparse_scalers:
        vel_train_split, 输入标准化器 = 构建数据切分(spec, train_cases, 特征列, args.train_velocity_source)
        p_train_split, _ = 构建数据切分(spec, train_cases, 特征列, args.train_pressure_source, input_scaler=输入标准化器)
        dense_train_split, _ = 构建数据切分(spec, train_cases, 特征列, "dense", input_scaler=输入标准化器)
        ...
        速度标准化器 = 输出标准化器.fit(vel_train_split.targets_raw[:, :2])
        压力标准化器 = 输出标准化器.fit(p_train_split.targets_raw[:, 2:3])
    else:
