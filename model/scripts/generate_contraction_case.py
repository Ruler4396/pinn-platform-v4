#!/usr/bin/env python3
"""Generate contraction_2d geometry/data artifacts for smoke and dataset-pipeline validation.

Important:
- The current dense field is a synthetic streamfunction-based reference used only to validate
  the directory/field/sampling pipeline.
- Formal experiments must replace it with CFD truth before training and reporting results.
"""
...
from src.data.contraction_cases import get_case, list_cases
from src.data.contraction_freefem import run_freefem
from src.data.contraction_geometry import ContractionGeometry, GridSpec
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate contraction_2d dataset artifacts")
    parser.add_argument("--case", default="C-base", help="case id, e.g. C-base")
    parser.add_argument("--list-cases", action="store_true", help="list known contraction cases")
    parser.add_argument("--nx", type=int, default=241, help="structured x resolution")
    parser.add_argument("--ny", type=int, default=121, help="structured y resolution")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="hard cap for future external generator retries; kept to avoid runaway consumption",
    )
    return parser


def finalize_freefem_dense_field(raw_df: pd.DataFrame, geometry: ContractionGeometry, case_id: str) -> pd.DataFrame:
    bc_map = {0: "interior", 1: "inlet", 2: "outlet", 3: "wall"}
    df = raw_df.copy()
    df["sample_id"] = range(len(df))
    df["case_id"] = case_id
    df["family"] = "contraction"
    ...
    ].assign(speed_star=lambda x: (x["u_star"] ** 2 + x["v_star"] ** 2) ** 0.5)[
        [
            "sample_id",
            "case_id",
            "family",
...
def main() -> None:
    ...
    if args.list_cases:
        for case in list_cases():
            print(f"{case.case_id}: beta={case.beta} Lc/W={case.lc_over_w} note={case.note}")
        return
    ...
    case_data_dir = root / "cases" / "contraction_2d" / "data" / case.case_id
    case_fig_dir = root / "cases" / "contraction_2d" / "figures" / case.case_id
    case_cfd_dir = root / "cases" / "contraction_2d" / "cfd" / case.case_id
    ...
    if args.field_source == "synthetic_streamfunction_smoke":
        ...
        edp_path, raw_csv = run_freefem(case, case_cfd_dir, max_retries=args.max_retries)
    ...
    if args.field_source == "synthetic_streamfunction_smoke":
        meta["warning"] = (
            "Synthetic smoke field only. Replace with CFD truth before formal training, "
            "evaluation, or thesis figures."
        )
    else:
        meta["warning"] = ""
        meta["cfd"] = {
    ...
    print(f"[done] generated contraction dataset artifacts for {case.case_id}")
