...
对应毕业设计《基于 PINN 的微流控芯片内二维稳态流场稀疏重建与可视化系统设计与实现》的整合仓库。它不只是一个模型训练脚本集——从参数化流道几何生真值，到构造稀疏/含噪观测，到双模型 PDE 耦合训练，到网页端完成场推理、稀疏重建和差异场对比，整条链路在同一仓库里可跑、可查、可复现；其中"重新生成 CFD 真值"那一层有目录前提（32 份入库 Stokes 脚本写死了原作者机器的绝对路径），见 [`docs/REPRODUCTION_GUIDE.md`](./docs/REPRODUCTION_GUIDE.md) §4 的可移植性边界。
...
本仓库已作为毕业论文修订的**工作区**。按导师审稿意见做的独立审计与施工方案放在下面三份文件里，接手前先读它们，不要从聊天记录里找上下文：

| 文件 | 内容 | 证据等级 |
| --- | --- | --- |
| [`docs/revision/独立审计报告-v1-20260924.md`](./docs/revision/独立审计报告-v1-20260924.md) | 效度审计：论文 57 条结论式声称逐条判档，14 条需撤回或改写；可辩护率 54% | 只读产物 + 本地重算 |
| [`docs/revision/修订方案-v1-20260924.md`](./docs/revision/修订方案-v1-20260924.md) | 7 天施工方案：任务矩阵、三个闸门、多种子统计方案、魔搭环境规约、附录 M「论文表 → run 目录 + 字段」映射 | 含独立复算 R1/R2/R3 |
| [`docs/revision/候选文献-20260924.md`](./docs/revision/候选文献-20260924.md) | 参考文献扩充候选 45 条（A 档 41），带三源核对记录与按节插入位置 | DOI 抽查 6/6 可解析 |

硬截止 **2026-09-30 结题**。执行环境：魔搭 CPU 实例（8 核 / 32 GB）。
